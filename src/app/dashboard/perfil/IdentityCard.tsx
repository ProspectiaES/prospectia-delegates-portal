...
function WeatherWidget() {
  const { weather, denied } = useWeather();
  if (denied || !weather) return null;
  const { label, icon } = wmoLookup(weather.code);
  return (
    <div className="flex items-center gap-1.5 mt-1 text-xs text-[#6B7280]">
      <span className="text-base leading-none">{icon}</span>
      <span className="font-semibold text-[#374151]">{weather.temp}°C</span>
      {weather.city && <><span className="text-[#9CA3AF]">·</span><span>{weather.city}</span></>}
