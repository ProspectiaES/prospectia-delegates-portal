...
export function EditOrderForm({ orderId, contactId, contactName, dateUnix, notes: initialNotes, initialLines, products }: Props) {
  ...
  const [lines, setLines] = useState<OrderLine[]>(initialLines.length > 0 ? initialLines : [{ key: 0, productId: "", name: "", units: 1, price: 0, discount: 0, taxes: [] }]);
    ...
    <form action={action} className="space-y-6">
      ...
      <section className="bg-white rounded-xl border border-[#E5E7EB] shadow-sm">
        ...
        <div className="divide-y divide-[#F3F4F6]">
          {lines.map((line, idx) => {
            const lineTotal = line.price * line.units * (1 - line.discount / 100);
            return (
              <div key={line.key} className="px-5 py-4 space-y-3">
                ...
                <input type="hidden" name="product_taxes[]" value={line.taxes.join(",")} />
                ...
                {/* Emit units/discount hidden for empty lines so indices stay aligned */}
                {!line.productId && (
                  <>
      ...
      <div className="flex items-center justify-between">
        <a href="/dashboard/pedidos" className="h-9 px-4 rounded-lg border border-[#E5E7EB] text-sm font-medium text-[#374151] hover:bg-[#F9FAFB] transition-colors flex items-center">
          Cancelar
        </a>
        <button
          type="submit"
